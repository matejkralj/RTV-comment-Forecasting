...
def lematiziraj_besedilo(besedila):
    """
    Lematizira seznam besedil kot batch, vrne seznam lematiziranih nizov.
    """
    dokumenti = nlp('\n\n'.join(besedila))
    lematizirani = []
    curr = []
    for sentence in dokumenti.sentences:
        leme = [word.lemma for word in sentence.words]
        curr.extend(leme)
        if sentence.text.strip() == '':
            lematizirani.append(' '.join(curr))
            curr = []
    if curr:  # še zadnji ostanek
        lematizirani.append(' '.join(curr))
    return lematizirani

def lemmatized_text(data, batch_size=16):
    print("Lematiziram besedilo v batchih...")
    for i in tqdm(range(0, len(data), batch_size)):
        batch = data[i:i+batch_size]
        paragrafi = [" ".join(clanek["paragraphs"]) for clanek in batch]
        lematizirani = lematiziraj_besedilo(paragrafi)
        for clanek, lemma_text in zip(batch, lematizirani):
            clanek["lemmatized_text"] = lemma_text
...
def extract(data):
    ...
    # Seznam državnih praznikov (mesec, dan)
    holidays = {
        (1, 1), (1, 2), (2, 8), (4, 27), (5, 1), (5, 2),  (6, 25),
        (10, 25), (11, 1), (12, 26)
    }
    ...
    for clanek in data:
        ...
        # Označevanje praznikov
        is_holiday.append(1 if (datum.month, datum.day) in holidays else 0)
    ...
    authors = [[a.get("authors", ["unknown"])[0]] for a in data]
    dates = [[a.get("date_days", 0)] for a in data]

    return (
        text, titles, content, topics, weekdays,
        authors, dates, sin_weekdan, cos_weekdan,
        sin_dayofyear, cos_dayofyear, sin_hour, cos_hour, is_holiday
    )
...
class RTVSlo:
    ...
    def fit(self, train_data: list):
        text, titles, content, topics, weekdays, authors, dates, sin_weekdan, cos_weekdan, sin_dayofyear, cos_dayofyear, sin_hour, cos_hour, is_holiday = extract(train_data)
        ...
        X_is_holiday = np.array(is_holiday, dtype=np.float32).reshape(-1, 1)
        ...
        X = sp.hstack([X_text_embeddings , X_titles, X_authors, X_pop_topic, X_pop_avtor, X_topics, X_sin_weekdan, X_cos_weekdan, X_sin_dayofyear, X_cos_dayofyear, X_sin_hour, X_cos_hour, X_is_holiday ]).astype(np.float32).toarray()
    ...
    def predict(self, test_data: list):
        ...
        text, titles, content, topics, weekdays, authors, dates,  sin_weekdan, cos_weekdan, sin_dayofyear, cos_dayofyear, sin_hour, cos_hour, is_holiday  = extract(test_data)
